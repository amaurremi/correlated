...
trait PropagationSpecBuilder extends Assertions with VariableFacts with IdeProblem { this: IdeSolver =>
      ...
      case (node, invokeInstr) =>
        targetStartNodes(node) foreach {
          getMethodName(_) match {
            case "shouldBeSecret"                   =>
              assertResult(Bottom)(getResultAtNode(node, invokeInstr))
            case "shouldNotBeSecret"                =>
              assertResult(Top)(getResultAtNode(node, invokeInstr))
  ...
  private[this] def getResultAtNode(node: Node, instr: SSAInvokeInstruction): LatticeElem = {
