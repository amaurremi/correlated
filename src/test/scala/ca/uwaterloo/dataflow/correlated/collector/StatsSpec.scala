...
class StatsSpec extends FunSpec {
  ...
  describe("Compute polymorphic calls") {
    ...
    it("Poly1: doesn't have polymorphic calls") {
      val stats = getAppCcStats("Poly1", path)
    ...
    it("Poly5: has 10 polymorphic calls") {
      val stats = getAppCcStats("Poly5", path)
    ...
    it("Poly6: has 6 polymorphic calls") {
      val stats = getAppCcStats("Poly6", path)
    ...
    it("Poly7: has 7 polymorphic calls") {
      val stats = getAppCcStats("Poly7", path)
    ...
    it("Poly8: has 1 polymorphic call (parameter passing)") {
      val stats = getAppCcStats("Poly8", path)
  ...
  describe("Precision of polymorphic call detection") {
    it("Poly9") {
      val stats = getAppCcStats("Poly9", path)
      assertResult(2, "dispatch call sites")(stats.dispatchCallSiteNum)
      assertResult(0, "polymorphic call sites")(stats.polymorphicCallSiteNum)
    }
  }

  describe("Programs containing only library calls") {
    it("Poly10") {
      val stats = getCcStats("Poly10", path)
      assertResult(1256, "dispatch call sites")(stats.dispatchCallSiteNum)
      assertResult((80, "polymorphic call sites"))(stats.polymorphicCallSiteNum)
      val appStats = getAppCcStats("Poly10", path)
      assertResult(1, "dispatch call sites")(appStats.dispatchCallSiteNum)
      assertResult(0, "polymorphic call sites")(appStats.polymorphicCallSiteNum)
    }
  }
}
