...
trait ExtraFeatures { self: AbstractCallGraphBuilder =>
  ...
  def findNode(pattern: String): Option[N] = {
    ...
    cg.asScala find {
      n =>
        n.getMethod.toString.matches(p)
    }
  }
  ...
  implicit def pWithPointerObjects(p: P) = new {
    def pt: Set[O] = (
      for {
        o <- heap.getSuccNodes(p).asScala
      } yield o.asInstanceOf[O]
    ).toSet
  }
  ...
  def localPtTo(o: O): Iterable[P] = (
    for {
      p <- heap.getPredNodes(o).asScala if p.isInstanceOf[P]
    } yield p.asInstanceOf[P]
  ).toIterable
}